...
anti_cancer = ['순무', '씀바귀', '우엉', '파프리카', '브로콜리', '가지', '표고버섯', '살구', '포도', '쪽파', '흑미',
               ...
               '꽃송이버섯', '버섯', '고등어', '꽁치', '정어리', '방어', '연어', '삼치', '참치', '청어', '전갱이']
#고혈압에 효과가 있는 재료들
high_blood_pressure = ['오만둥이', '마늘', '메밀', '양송이버섯', '여주', '올리브', '양파', '연근', '녹두', '더덕',
                       '토마토', '고구마', '강낭콩', '연어', '바나나', '아몬드', '호박씨', '목이버섯', '비트', '송이버섯',
                       '가지', '감자', '파프리카', '브로콜리', '당근', '피망', '참나물', '고들빼기', '팥']
#당뇨에 효과가 있는 재료들
diabetes = ['여주', '녹두', '더덕', '토마토', '강낭콩', '아몬드']
...
def makefooddatabase(pagebaseurl, lastpagenum, aorw):
    if aorw == "w":     
        print("새로쓰기 모드로 작성")
        f = open('newexample2.csv', 'w', encoding='ms949', newline='')   #두번째를 'a'로 하면 이어쓰기, 'w'로 하면 새로 쓰기
        csvwriter = csv.writer(f)
        csvwriter.writerow(['이름', 'id', '대분류', '소분류', '재료', '건강정보', 'link'])    #데이터베이스 첫줄
    elif aorw == "a":
        print("이어쓰기 모드로 작성")
        f = open('newexample2.csv', 'a', encoding='ms949', newline='')
        csvwriter = csv.writer(f)
